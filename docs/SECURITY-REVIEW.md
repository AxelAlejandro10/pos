...
| Public waiting list | **Create:** `POST /public/tenants/{tenant_id}/waiting-list` — unauthenticated; tenant-scoped. Accepts guest **PII** (name, E.164 phone) + party size; stores client IP/UA on the entry. **Rate-limited** separately: `RATE_LIMIT_WAITING_LIST_PER_HOUR` (default **10/hour/IP**) — see `docs/0020-rate-limiting-production.md`. No public token / status page (staff contact the guest; staff APIs use `reservation:read` / `reservation:write`). Feature guide: `docs/0011-table-reservation-user-guide.md`. |
| Public Satisfecho Delivery | **Create:** `POST /public/tenants/{tenant_id}/satisfecho-delivery` — unauthenticated; **rate-limited** (`@public_menu_ip_limit`, same bucket as public menu — see `docs/0020-rate-limiting-production.md`). Requires address + E.164 phone. **Item IDs (#304):** each line `product_id` may be a tenant-scoped public-menu **`TenantProduct.id`** (resolved to the linked `Product` via `delivery_order_service`) or a legacy **`Product.id`**; both paths require `tenant_id` match — cross-tenant IDs must not resolve. On create, validates **postal and/or radius** coverage (`validate_delivery_coverage`); **delivery fee** is snapshotted on the order from tenant settings. Returns signed `public_order_token` (**24h** — pay + track) and payment hints (`stripe_publishable_key`, `revolut_configured` — publishable only). Kitchen/inventory notify is **deferred until payment succeeds**. **Config (no secrets):** `GET /public/tenants/{tenant_id}/satisfecho-delivery-config` — fee + coverage hints; same IP rate limit. **Track:** `GET /public/orders/{order_id}/delivery-status?public_order_token=` — token-gated coarse status only (no maps); same IP rate limit. Staff create (`POST /orders/satisfecho-delivery`) is authenticated and notifies immediately. Feature doc: `docs/0053-satisfecho-delivery-order-channel.md`. Tests: `back/tests/test_public_satisfecho_delivery.py` (incl. `test_public_create_accepts_tenant_product_menu_ids` for #304). |
...
| Restaurant groups | Authenticated **owner/admin** only (`/restaurant-group` create / join / leave / settings). **`join_code`** is a **capability secret** for the group (anyone with the code can attach their tenant). Optional `share_products` / `share_customers` expand product/customer visibility across member tenants — treat join-code leakage and share flags as cross-tenant data-boundary controls. Feature doc: `docs/0054-restaurant-groups.md`. Tests: `back/tests/test_restaurant_groups.py`. |
...
- **Unpaid public delivery orders:** Create persists an order before pay; kitchen is deferred. **TTL cleanup implemented** (default **2h** unpaid-create cleanup — shorter than the **24h** `public_order_token` used for pay + track): `python -m app.seeds.cleanup_unpaid_public_delivery` cancels tagged public creates (`session_id=public_satisfecho_delivery`) that stay unpaid; staff creates are never cleaned. See `docs/0053-satisfecho-delivery-order-channel.md`. Rate limits still cap create / config / track-poll rate.
- **Dual product ID space on public delivery create:** Intentional for catalog menus (`TenantProduct.id` vs `Product.id`). Control remains tenant-bound resolution — do not accept cross-tenant IDs; keep the #304 regression test green when changing line resolution.
...
- `docs/0011-table-reservation-user-guide.md` (public waiting list)
- `docs/0020-rate-limiting-production.md` (public menu / delivery / webhook / waiting-list buckets)
...
| 2026-07-23 | Delta for **2.1.32 / #306**: documented zone/fee validation on public create, `GET …/satisfecho-delivery-config`, token-gated `GET …/delivery-status` (track), and corrected `public_order_token` lifetime to **24h** (unpaid create TTL remains **2h**). Spot-checked `@public_menu_ip_limit` on config + status. **Not a penetration test.** |
| 2026-07-23 | Delta for **#304**: Public Satisfecho Delivery create item IDs may be `TenantProduct.id` (resolved to linked `Product`) or legacy `Product.id`, both tenant-bound; residual note on dual ID space; cite `test_public_create_accepts_tenant_product_menu_ids`. **Not a penetration test.** |
| 2026-07-23 | Delta: documented **public waiting list** (`POST /public/tenants/{tenant_id}/waiting-list` — PII, `RATE_LIMIT_WAITING_LIST_PER_HOUR`, no public token page) and **restaurant groups** (`join_code` as capability secret; `share_products` / `share_customers` cross-tenant visibility; owner/admin only). Linked `docs/0011`, `docs/0020`, `docs/0054`, `test_restaurant_groups.py`. **Not a penetration test.** |
