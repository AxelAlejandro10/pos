...
1. **Capacity and demand**
   ...
   - **Average table turn (tenant `reservation_average_table_turn_minutes`):** when set, a table with a seated reservation (see `reservation.seated_at`), an in-progress order, or `is_active` is treated as busy only for `[start, start + turn)` in UTC (`start` = earliest of seated_at / order.created_at / activated_at / now). When unset, same-day behaviour stays a simple all-day block for busy tables.
   - **Walk-in buffer (`reservation_walk_in_tables_reserved`):** after computing which tables are free at the slot, the smallest tables (by `seat_count`, then `id`) are removed from the reservation pool so that many remain for walk-ins.
