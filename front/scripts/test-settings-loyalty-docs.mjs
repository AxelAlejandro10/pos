#!/usr/bin/env node
/**
 * Puppeteer smoke: Settings → Loyalty club docs link (#394).
 * Asserts the club loyalty runbook link points at docs/0066 on GitHub.
 *
...
const EXPECTED_HREF =
  'https://github.com/satisfecho/pos/blob/master/docs/0066-club-loyalty.md';
...
async function main() {
  ...
  try {
    ...
    console.log('2. Opening Settings → Loyalty club...');
    await page.goto(new URL('/settings', baseUrl).href, {
      waitUntil: 'networkidle2',
      timeout: 15000,
    });
    await sleep(2000);
    const printingTab = await page.$('[data-testid="settings-loyalty-tab"]');
    if (!printingTab) {
      console.log('   FAIL: Loyalty club tab not found.');
      await browser.close();
      process.exit(1);
    }
    ...
    const link = await page.waitForSelector('[data-testid="settings-loyalty-docs-link"]', {
      timeout: 10000,
    }).catch(() => null);
    if (!link) {
      console.log('   FAIL: settings-loyalty-docs-link not found.');
      await browser.close();
      process.exit(1);
    }
